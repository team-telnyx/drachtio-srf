...
class DrachtioAgent extends Emitter {
  ...
  _onMsg(socket, msg) {
    ...
    if (!token || token.length < 2) {
      debug(`DrachtioAgent#_onMsg: invalid message format, token array too short: ${JSON.stringify(token)}`);
      debug(`DrachtioAgent#_onMsg: original message: ${msg.slice(0, Math.min(500, msg.length))}`);
      return;
    }

    // Check if token[1] is a valid message type
    const validTypes = ['sip', 'response', 'cdr:attempt', 'cdr:start', 'cdr:stop'];
    if (!token[1] || !validTypes.includes(token[1])) {
      debug(`DrachtioAgent#_onMsg: invalid message type '${token[1]}', expected one of: ${validTypes.join(', ')}`);
      debug(`DrachtioAgent#_onMsg: message tokens: ${JSON.stringify(token.slice(0, 5))}`);
      debug(`DrachtioAgent#_onMsg: message preview: ${msg.slice(0, Math.min(200, msg.length))}`);
      return;
    }
